...
/**
 * 文件名：setting.mod.php
 * @version $Id: settings.mod.php 5526 2014-02-10 08:55:10Z wuliyong $
 * 作者：狐狸<foxis@qq.com>
 * 功能描述: 个人设置模块
 */
...
class ModuleObject extends MasterObject
{
		...
		function DoUserTag()
	{
		...
		$tag_name_arr = wap_iconv($tag_name_arr,'utf-8',$this->Config['charset'], 1);
	...
	function DoModifyProfile()
	{
		...
		$member = wap_iconv($member);
				...
				$aboutme = strlen(($aboutme = jpost('aboutme', 'txt'))) > 255 ? cutstr($aboutme,254).'': $aboutme;

				$signature = strlen(($signature = jpost('signature', 'txt'))) > 48 ? cutstr($signature,48).'': $signature;


				if(($filter_msg = filter($aboutme)))
        {
            $this->Messager('【个人说明错误】' . $filter_msg['msg'],'index.php?mod=settings');
        }

        		if(($filter_msg = filter($signature)))
        {
            $this->Messager('【个人签名错误】' . $filter_msg['msg'],'index.php?mod=settings');
        }

        		if(($filter_msg = filter($nickname)))
        {
            $this->Messager('【个人昵称错误】' . $filter_msg['msg'],'index.php?mod=settings');
        }
