...
class ModuleObject extends MasterObject
{
	...
	function eventList(){
		...
		include(template('event/event_list'));
	...
	function _makeSimpleHtml($arr){
		...
		foreach ($arr as $key=>$val) {
			...
			$html .="<tr>";
			if($profile[$key]['formtype'] == 'select' && $profile[$key]['choices']){
				$detail=explode("\r\n",$profile[$key]['choices']);
				foreach ($detail as $val1){
					list($v1,$v2) = explode("|",$val1);
					$v2 || $v2 = $v1;
					$_html .= "<option value=\"$v1\">$v2</option>";
				}
				$html .= "<td>".$profile[$key]['title'].":<span style=\"color:red\">*</span></td>
						  <td>
						    <select id=\"".$profile[$key]['fieldid']."\" name=\"".$profile[$key]['fieldid']."\">
						     $_html
						    </select>
						  </td>";
				$_html = "";
			} else {
				$html .= "<td>".$profile[$key]['title'].":<span style=\"color:red\">*</span></td><td><input type=\"text\" id=\"".$profile[$key]['fieldid']."\" name=\"".$profile[$key]['fieldid']."\"></td>";
			}
			$html .="</tr>";
		}
		$html .="<tr>
