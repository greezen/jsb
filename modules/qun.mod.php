...
class ModuleObject extends MasterObject
{
	...
	function create()
	{
		...
		$this->Title = '创建新'.$this->Config[changeword][weiqun];
		$detail = array();
		$is_allowed = true;
				if (MEMBER_ROLE_TYPE != 'admin') {
	...
	function docreate()
	{
		...
		if (MEMBER_ROLE_TYPE != 'admin') {
			...
			$is_allowed = $this->QunLogic->allowed_create(MEMBER_ID, $detail);
			if ($is_allowed == false) {
				$this->Messager("不允许创建".$this->Config[changeword][weiqun]);
			}
		}

		$member = $GLOBALS['_J']['member'];
		$post['username'] = addslashes($member['username']);
		$post['uid'] = $member['uid'];

		$province = trim(DB::result_first("select name from ".TABLE_PREFIX."common_district where id = '".$this->Post['province']."'")); 		$post['province'] = $province;
		$city = trim(DB::result_first("select name from ".TABLE_PREFIX."common_district where id = '".$this->Post['city']."'"));		$post['city'] = $city;

		$ret = $this->QunLogic->create($post);

		if ($ret == 1) {
			$this->Messager("创建".$this->Config[changeword][weiqun]."成功了", 'index.php?mod=qun&code=invite&qid='.$post['qid']);
		} else if ($ret == -1) {
			$this->Messager($this->Config[changeword][weiqun]."名称为空或者名称长度小于3", -1);
		} else if ($ret == -2) {
			$this->Messager($this->Config[changeword][weiqun] . "分类为空或者不存在", -1);
		} else if ($ret == -3) {
			$this->Messager($this->Config[changeword][weiqun] . "所在省份或者城市为空", -1);
		}
		exit;
	}
	...
	function profile()
	{
		...
		$view_list = array(
				'managed' => array(
					'title' => '我管理的'.$this->Config[changeword][weiqun],
					'code' => 'managed',
				),
				'joined' => array(
					'title' => '我加入的'.$this->Config[changeword][weiqun],
					'code' => 'joined',
				),
				'followed' => array(
					'title' => '我关注的人的'.$this->Config[changeword][weiqun],
					'code' => 'followed',
				)
		);
		...
		if (in_array($view, array('managed', 'joined', 'followed'))) {
			...
			unset($tmp);
			$this->Title = '我的'.$this->Config[changeword][weiqun];
		}
		$member = &$this->my;
	...
	function manage()
	{
		...
		$this->Title = $this->Config[changeword][weiqun].'管理 - '.$qun_info['name'];
		if ($op == 'icon') {
			$qun_info['icon'] = $this->QunLogic->qun_avatar($qun_info['qid'], 'b');
			$u_tips = $this->QunLogic->upload_tips();
		} else if ($op == 'privacy') {
	...
	function domanage()
	{
		...
		if ($op == 'setting') {
			...
			$province = trim(DB::result_first("select name from ".TABLE_PREFIX."common_district where id = ".(int) $this->Post['province'])); 			$post['province'] = $province;
			$city = trim(DB::result_first("select name from ".TABLE_PREFIX."common_district where id = ".(int) $this->Post['city']));			$post['city'] = $city;

			$ret = $this->QunLogic->modify_setting($this->Post);
			if ($ret == -1) {
				$this->Messager("名称不能小于3个字节");
			} else if ($ret == -2) {
				$this->Messager("您还没有选择分类呢");
			} else if ($ret == -3) {
				$this->Messager("您还没有选择所在地呢");
			}
		} else if ($op == 'icon') {
			...
			$join_type = (int) $this->Post['join_type'];
			$join_type = in_array($join_type, array(0, 1, 2)) ? $join_type : 0;
			$set_ary = array(
			...
			if (empty($qun_user)) {
				$this->Messager("当前用户不是".$this->Config[changeword][weiqun]."内成员，无法对他进行操作");
			}
		...
		function ploy()
	{
		...
		$this->Title = $this->Config[changeword][weiqun]."策略";
