...
class ModuleObject extends MasterObject {
	...
	public function index() {
		...
		$order = $rets['order'];//var_dump($order);exit;
	...
	public function today_order() {
		...
		$p = array(
			...
			'perpage' => (int) jconf::get('show', 'topic_new', 'topic'),
			'page_url' => 'index.php?mod=plaza&code=today_order',
		);
		$rets = jlogic('plaza')->order('today',$p);

		$ajax_load = $this->_ajax_load($rets);
		$GLOBALS['_J']['navcss']['todayorder'] = 'current';
		include template();
	}
	
	//##lbc##推荐订单
	public function dig_order() {
		$this->Title = '推荐订单';
		...
		$p = array(
			...
			'perpage' => (int) jconf::get('show', 'topic_new', 'topic'),
			'page_url' => 'index.php?mod=plaza&code=dig_order',
		);
		$rets = jlogic('plaza')->order('dig', $p);

		$ajax_load = $this->_ajax_load($rets);
		$GLOBALS['_J']['navcss']['digorder'] = 'current';
		include template('plaza_today_order');
	}
	
	//##lbc##急需加工的订单
	public function maf_order() {
		$this->Title = '急需加工';
		...
		$p = array(
			...
			'perpage' => (int) jconf::get('show', 'topic_new', 'topic'),
			'page_url' => 'index.php?mod=plaza&code=maf_order',
		);
		$rets = jlogic('plaza')->order('maf', $p);

		$ajax_load = $this->_ajax_load($rets);
		$GLOBALS['_J']['navcss']['maforder'] = 'current';
		include template('plaza_today_order');
	}
	
	//##lbc##急需加工的订单
	public function coop_order() {
		$this->Title = '长期合作';
		...
		$p = array(
			...
			'perpage' => (int) jconf::get('show', 'topic_new', 'topic'),
			'page_url' => 'index.php?mod=plaza&code=coop_order',
		);
		$rets = jlogic('plaza')->order('coop', $p);

		$ajax_load = $this->_ajax_load($rets);
		$GLOBALS['_J']['navcss']['cooporder'] = 'current';
		include template('plaza_today_order');
	}
