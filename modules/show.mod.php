		...
		function Main()
	{
		...
		if($uid > 0) {
			...
			$isBorder		= get_param('isBorder') ? '1' : '';
				...
				$p = array(
					'uid' => $uid,
					'result_count' => $userlimit,
					'sql_order' => ' `dateline` DESC ',
				);
				$uids = jlogic('buddy')->get_fansids($p);

    			$user_list = $this->TopicLogic->GetMember("where `uid` in(".jimplode($uids).") limit {$userlimit}","`uid`,`ucuid`,`username`,`nickname`,`face_url`,`face`,`topic_count`,`fans_count`,`validate`,`province`,`city`");
    		}
