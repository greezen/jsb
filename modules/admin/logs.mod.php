 ...
 *
 * @Date 2013-11-11 1363708007 5253 $
 */
...
class ModuleObject extends MasterObject
{
	...
	function View() {
		...
		$msg = "<div style='text-align: left;'>
			...
			<b>IP地址:端口</b>{$info[ip]}:{$info[ip_port]}<br /><br />
	...
	function Clean() {
		$month = jget('month', 'int');
		if(!in_array($month, array(3, 6, 12))) {
			$month = 6;
		}
		
				if($month > 0) {
			$time = (TIMESTAMP - 86400 * 30 * $month);
