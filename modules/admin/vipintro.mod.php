...
class ModuleObject extends MasterObject
{
	...
	function VipConditionSet(){
		$is_edit = jget('is_edit','int');
		if($is_edit){
			$condition = array();
			$forward = jget('forward');
			$conditions = array(
				'email'=>array(
							'key'=>'email',
							'msg' => '本站需通过Email验证才能申请V认证'
						),
				'topic_num'=>array(
							'key'=>'topic_num',
							'msg' => '本站需发'.jget('topic_num','int').'条内容才能申请V认证'
						),
				'face'=>array(
							'key'=>'face',
							'msg' => '本站需上传头像才能申请V认证'
				...
				'fans_num'=>array(
							'key'=>'fans_num',
							'msg' => '本站需'.jget('fans_num','int').'位粉丝才能申请V认证'
						),
				'city'=>array(
							'key'=>'city',
							'msg' => '本站需设置所在区域才能申请V认证'
						),
			);
			...
			foreach ($conditions as $key => $val) {
				...
				$condition[$key] = array(
					'key' => $key,
					'forward' => $forward[$key] ? $forward[$key] : '',
					'enable' => $enable,
					'message' => $val['msg'],
				);
			}
			jconf::set('vipcondition',$condition);
			$this->Messager('设置成功','admin.php?mod=vipintro&code=setting');
		}
		$vipConditions = jconf::get('vipcondition');
		$email = $this->jishigou_form->YesNoRadio('email', $vipConditions['email']['enable']);
		$topic_num = $vipConditions['topic_num']['enable'];
		$face = $this->jishigou_form->YesNoRadio('face', $vipConditions['face']['enable']);
		$fans_num = $vipConditions['fans_num']['enable'];
		$city = $this->jishigou_form->YesNoRadio('city', $vipConditions['city']['enable']);
		include template('admin/vip_condition');
	}
	...
	function doAddVip(){
		...
		if($nickname_arr) {
			foreach ($nickname_arr as $k=>$name) {
				$name = trim($name);
				$member_info = DB::fetch_first("select `uid`, `username`, `nickname`, `city`, `province` from `".TABLE_PREFIX."members` where `nickname`='$name'");
				$uid = $member_info['uid'];
				if($uid < 1) {
					$f_name[] = $name.':不存在';
					continue;
				...
				if(empty($validate_info)) {
					...
					if($vdesc) {
						jtable('memberfields')->update(array('validate_remark'=>$vdesc), $uid);
					}
		...
		$msg = '添加' . ($true ? '成功' : '失败') . ($f_name ? ('，其中 ' . implode('<br />',$f_name)) : '');
		$this->Messager($msg);
	}
	...
	function Main()
	{
		...
		include template('admin/vipintro');
	}
		...
		function MemberView()
	{
		...
		include template('admin/vipintro_user');
	}
		...
		function Modify()
	{
		...
		$validate_card_type_select = $this->jishigou_form->Select('validate_card_type',$_options,$memberfields['validate_card_type']);
		...
		include template('admin/modify_vipintro');
	}
	...
	function CategoryList()
	{
		...
		include template('admin/validate_category');
	}
	...
	function &get_category_tree()
	{
		...
		jconf::set('validate_category',$cat_ary);
	...
	function DoCategory()
	{
				...
				if($this->Post['postFlag'])
		{
			...
			jconf::get('validate_category',$validate_category);
	...
	function uploadPic($field,$category_pic_id) {
		...
		$fext = jio()->FileExt($_FILES[$field]['name']);
		if(!in_array($fext, array('jpg', 'jpeg', 'gif', 'png', ))) {
			$fext = 'gif';
		}

		$image_path = RELATIVE_ROOT_PATH . 'images/' . $field . '/'. face_path($category_pic_id);
		$image_name = $category_pic_id . "_o." . $fext;
