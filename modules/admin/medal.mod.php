...
/**
 * 文件名：topic.mod.php
 * @version $Id: medal.mod.php 5639 2014-03-11 02:04:49Z chenxianfeng $
 * 作者：狐狸<foxis@qq.com>
 * 功能描述: 微博模块
 */
...
class ModuleObject extends MasterObject
{
	...
	function Main()
	{
		...
		$count = DB::result_first("select count(*) from ".TABLE_PREFIX."medal_apply a where 1 $where ");
		...
		include template('admin/medal_verify');
	}

		function doVerify()
	{
		$uid = (int) $this->Get['uid'];
		if($uid < 1 || !($user = jsg_member_info($uid))) {
			$this->Messager('用户已经不存在了');
		}
		$medal_id = (int) $this->Get['medal_id'];
		if($medal_id < 1) {
			$this->Messager('勋章ID不能为空');
		}
		...
		function Add()
	{
		...
		if(empty($this->Post['postFlag']))
		{
			...
			include template('admin/medal_info');
		}
		else
		{
		...
		function DoView()
	{
		...
		$count = DB::result_first("select count(*) from ".TABLE_PREFIX."user_medal a where 1 $where ");
		$page_arr = page($count,$per_page_num,$page_url,$_config);

		$sql = "select a.medalid as medal_id,
					a.uid,
					a.nickname,
					a.dateline,
					i.medal_img,
		...
		include template('admin/medal_view');
	}
		...
		function Delete()
	{
		...
		$this->DatabaseHandler->Query($sql);
		$sql = "UPDATE `".TABLE_PREFIX."members` SET medal_id = TRIM(BOTH ',' FROM REPLACE(CONCAT(',',medal_id,','),',{$ids},',',')) where CONCAT(',',medal_id,',') LIKE '%,{$ids},%'";
		$this->DatabaseHandler->Query($sql);
		...
		function DelMdealUser()
	{
		...
		$this->DatabaseHandler->Query($sql);
				...
				$sql = "UPDATE `".TABLE_PREFIX."members` SET medal_id = TRIM(BOTH ',' FROM REPLACE(CONCAT(',',medal_id,','),',{$medalid},',',')) where uid = '{$uid}'";
		$this->DatabaseHandler->Query($sql);
