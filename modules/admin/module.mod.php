...
class ModuleObject extends MasterObject
{
	...
	function main(){
		...
		while($rs = $query->GetRow()){
			$num=DB::result_first("SELECT count(*) AS NUM FROM " . TABLE_PREFIX . "fenlei_sort WHERE mid='$rs[id]' ");
		...
		function editOrder(){
		$id = (int) $this->Get['id'];
		$rs=DB::result_first("SELECT config FROM " . TABLE_PREFIX . "fenlei_module WHERE id='$id' ");
		...
		function deleteSort(){
		$id = jget('id', 'int');
		$num=DB::result_first("SELECT count(*) AS num FROM " . TABLE_PREFIX . "fenlei_sort WHERE mid='$id'");
		if($num){
		...
		if($id){
			$table = TABLE_PREFIX . "fenlei_content_" . $id;
		    			$config2 = DB::result_first("select config from `".TABLE_PREFIX."fenlei_module` where `id` = '$id'");
		...
		$config= addslashes(serialize($array));
			...
			if($array[IfListShow][$key]){
				$Temp_list_rs.="<li>{\$rs[{$key}]}</li> ";
				if($rs[form_units]){
					$add = "(".$rs[form_units].")";

				}
				$Temp_list_top.="<li>{$rs[name]}$add</li> ";
			}
		}
		...
		$count = DB::result_first("select count(*) from ".TABLE_PREFIX."fenlei_ziduan where field_name = '$postList[field_name]'");
		if($count){
		...
		$rsdb = array();
		$disabled = "disabled";
		include(template("/fenlei/post_$mid"));
	}
