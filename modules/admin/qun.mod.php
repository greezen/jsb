...
class ModuleObject extends MasterObject
{
	...
	function module(){
	  	$qun_module = jconf::get('qun_module');
	  	$max_id = 0;
	...
	function event(){
		...
		$count = DB::result_first("select count(*) from ".TABLE_PREFIX."event a where 1 $where and verify = 1 ");
		if($count){
			$page_arr = page($count,$per_page_num,$page_url,array('return'=>'array',),'20 50 100 200,500');
			$sql .= $page_arr['limit'];
	...
	function setting()
	{
		$config_all = jconf::get();
		$config = $config_all['qun_setting'];
		$checked = array();
		$checked['qun_open'][$config['qun_open'] ? $config['qun_open'] : 0] = 'CHECKED';
		$checked['new_qun'][$config['new_qun'] ? $config['new_qun'] : 0] = 'CHECKED';
		$checked['tc_qun'][$config['tc_qun'] ? $config['tc_qun'] : 0] = 'CHECKED';
        $checked['qun_topic_to_weibo'][($config['qun_topic_to_weibo'] ? $config['qun_topic_to_weibo'] : 0)] = 'CHECKED';
