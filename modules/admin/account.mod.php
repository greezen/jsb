 ...
 * @version $Id: account.mod.php 4773 2013-10-31 09:24:18Z wuliyong $
 */
...
class ModuleObject extends MasterObject
{
	...
	function Execute()
	{
            ...
            if($this->Code){
                ...
                if(!in_array($this->Code, array('on_off','index'))){
                    $this->Messager("需要开启允许同步到第三方后方可设置");
                }
            }
        }
		ob_start();
	...
	function Main()
	{
		...
		if($item_list_config) {
			...
			$item_list_radio = $this->jishigou_form->Radio('item', $item_list_config, $item);
		}
    ...
    function onOff(){
        $param = (int)get_param('account_on_off');
        jconf::update('account_on_off', $param);
	...
	function YY()
	{
		...
		$yy_enable_radio = $this->jishigou_form->YesNoRadio('yy[enable]', (int) ($yy['enable'] && $this->Config['yy_enable']));
		$yy_reg_pwd_display_radio = $this->jishigou_form->YesNoRadio('yy[reg_pwd_display]', (int) $yy['reg_pwd_display']);
	...
	function DoModifyYY()
	{
		...
		if($rets)
		{
			jconf::update('yy_enable', 0);

			$this->Messager($rets, null);
		}


		$yy_default = jconf::get('yy');
		...
		{
			jconf::update('yy_enable', $yy['enable']);
		}
	...
	function Renren()
	{
		...
		$renren_enable_radio = $this->jishigou_form->YesNoRadio('renren[enable]', (int) ($renren['enable'] && $this->Config['renren_enable']));
		$renren_reg_pwd_display_radio = $this->jishigou_form->YesNoRadio('renren[reg_pwd_display]', (int) $renren['reg_pwd_display']);
		$renren_is_sync_topic_radio = $this->jishigou_form->YesNoRadio('renren[is_sync_topic]', (int) $renren['is_sync_topic']);
		$renren_is_sync_image_radio = $this->jishigou_form->YesNoRadio('renren[is_sync_image]', (int) $renren['is_sync_image']);
	...
	function DoModifyRenren()
	{
		...
		if($rets)
		{
			jconf::update('renren_enable', 0);

			$this->Messager($rets, null);
		}


		$renren_default = jconf::get('renren');
		...
		{
			jconf::update('renren_enable', $renren['enable']);
		}


		if($renren != $renren_default)
		{
			jconf::set('renren', $renren);
		}
	...
	function Kaixin()
	{
		...
		$kaixin_enable_radio = $this->jishigou_form->YesNoRadio('kaixin[enable]', (int) ($kaixin['enable'] && $this->Config['kaixin_enable']));
		$kaixin_reg_pwd_display_radio = $this->jishigou_form->YesNoRadio('kaixin[reg_pwd_display]', (int) $kaixin['reg_pwd_display']);
		$kaixin_is_sync_topic_radio = $this->jishigou_form->YesNoRadio('kaixin[is_sync_topic]', (int) $kaixin['is_sync_topic']);
		$kaixin_is_sync_image_radio = $this->jishigou_form->YesNoRadio('kaixin[is_sync_image]', (int) $kaixin['is_sync_image']);
	...
	function DoModifyKaixin()
	{
		...
		if($rets)
		{
			jconf::update('kaixin_enable', 0);

			$this->Messager($rets, null);
		}


		$kaixin_default = jconf::get('kaixin');
		...
		{
			jconf::update('kaixin_enable', $kaixin['enable']);
		}


		if($kaixin != $kaixin_default)
		{
			jconf::set('kaixin', $kaixin);
		}
	...
	function Baidu()
	{
		...
		$baidu_enable_radio = $this->jishigou_form->YesNoRadio('baidu[enable]', (int) ($baidu['enable'] && $this->Config['baidu_enable']));
		$baidu_reg_pwd_display_radio = $this->jishigou_form->YesNoRadio('baidu[reg_pwd_display]', (int) $baidu['reg_pwd_display']);
	...
	function DoModifyBaidu()
	{
		...
		if($rets)
		{
			jconf::update('baidu_enable', 0);

			$this->Messager($rets, null);
		}


		$baidu_default = jconf::get('baidu');
		...
		{
			jconf::update('baidu_enable', $baidu['enable']);
		}


		if($baidu != $baidu_default)
		{
			jconf::set('baidu', $baidu);
		}
	...
	function Fjau()
	{
		...
		$fjau_enable_radio = $this->jishigou_form->YesNoRadio('fjau[enable]', (int) ($fjau['enable'] && $this->Config['fjau_enable']));
		$fjau_reg_pwd_display_radio = $this->jishigou_form->YesNoRadio('fjau[reg_pwd_display]', (int) $fjau['reg_pwd_display']);
	...
	function DoModifyFjau()
	{
		...
		if($rets)
		{
			jconf::update('fjau_enable', 0);

			$this->Messager($rets, null);
		}


		$fjau_default = jconf::get('fjau');
		...
		{
			jconf::update('fjau_enable', $fjau['enable']);
		}
	...
	function HZSWB()
	{
		...
		$hzswb_enable_radio = $this->jishigou_form->YesNoRadio('hzswb[enable]', (int) ($hzswb['enable'] && $this->Config['hzswb_enable']));
		$hzswb_auto_reg_bind_radio = $this->jishigou_form->YesNoRadio('hzswb[auto_reg_bind]', (int) $hzswb['auto_reg_bind']);


		include template('admin/account_hzswb');
	}
