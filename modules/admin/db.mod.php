 ...
 * @version $Id: db.mod.php 4029 2013-07-25 08:56:40Z wuliyong $
 */
...
class ModuleObject extends MasterObject
{
	...
	function Import()
	{
		...
		if (!$backupdir) {
			if(true===JISHIGOU_FOUNDER) {
				$_f_list = (array) jio()->ReadDir(RELATIVE_ROOT_PATH.'data/backup/db/'.$backupdir,1);
				$f_list = array();
				$key = 0;
		...
		include template('admin/db_import');
	}
	...
	function optimize()
	{
		...
		include template('admin/db_optimize');
	}
	function DoOptimize()
	{
	...
	function Export()
	{
		...
		include template('admin/db_export');
	}
	function DoExport()
	{
...
".(true===JISHIGOU_FOUNDER ? $filelist : "<br />文件备份在  data/backup/db/ 目录下") ,null);
	...
	function _sql_dump_table($table, $startfrom = 0, $currsize = 0) {
		...
		if(strexists($table, 'sessions') || strexists($table, 'cache')) {
			return ;
