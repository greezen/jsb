...
class ModuleObject extends MasterObject
{
	...
	function main(){
		$id = (int) $this->Get['id'];
		if($id){
			$province = DB::result_first("select name from `".TABLE_PREFIX."common_district` where id = '{$id}'");
		}
	...
	function city(){
		...
		$id = (int) $this->Get['id'];
		if($id){
			$name = DB::result_first("select name from `".TABLE_PREFIX."common_district` where id = '{$id}'");
		}
