...
class ModuleObject extends MasterObject
{
	...
	function ModuleObject($config)
	{
		...
		$this->TopicListLogic = jlogic('topic_list');
	...
	function UserSearch()
	{
		...
		if($tags) {
			...
			if ($keyword) {
				...
				$where_list['keyword'] = (false!==strpos($keyword,'@') ?
					"`email`='".addslashes("{$keyword}")."'" :
					build_like_query("`nickname`, `province`, `city`",$keyword));
		...
		function TopicSearch()
	{
		...
		if($keyword)
		{
			...
			$cache_time = 600;
			$tids = jlogic('topic_list')->get_tids_by_keyword($keyword, $this->cache_ids_limit, $cache_time);
