 ...
 * @version $Id: db.class.php 4774 2013-11-01 01:19:03Z wuliyong $
 */
...
class cache_db {
	...
	function cache_db() {
		...
		$this->num = max(16, (int) $_J['config']['cache_table_num']);
	...
	function clean() {
		DB::query("TRUNCATE TABLE ".DB::table($this->table), "SILENT");
				if($this->num) {
			for ($i=1; $i<$this->num; $i++) {
				DB::query("TRUNCATE TABLE ".DB::table($this->_sub_table($i)), "SILENT");
							}
		}
		if($this->memory) {
			$this->memory->clear();
