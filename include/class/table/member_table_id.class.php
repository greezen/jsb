 ...
 * @version $Id: member_table_id.class.php 3678 2013-05-24 09:48:20Z wuliyong $
 */
...
class table_member_table_id extends table {

	
	var $table = 'member_table_id';

	function table_member_table_id() {
		$this->init($this->table);
	}

	function row($uid, $is_dateline = 0) {
		$ret = false;
		$uid = jfilter($uid, 'int');
		if($uid > 0) {
												$maps = $this->get_maps();
	...
	function table_id($uid, $by_dateline = 0) {
		$ret = false;
		$row = $this->row($uid, $by_dateline);
		if($row) {
			$ret = $row['id'];
		}
		return $ret;
