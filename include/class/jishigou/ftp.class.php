 ...
 * @version $Id: ftp.class.php 3740 2013-05-28 09:38:05Z wuliyong $
 */
...
class jishigou_ftp
{
	...
	function jishigou_ftp() {
		;
	}

	function init($ftpkey=0) {
		$this->set_error(0);
		$ftp_config = jconf::get('ftp');
		$this->config = $ftp_config[$ftpkey];
		$this->enabled = false;
		if(empty($this->config['on']) || empty($this->config['host'])) {
			...
			if($this->func == 'ftp_connect' && !function_exists('ftp_connect')) {
				...
				$this->config['host'] = jishigou_ftp::clear($this->config['host']);
	...
	function ftp_mkdir($directory) {
		$directory = jishigou_ftp::clear($directory);
	...
	function ftp_put($remote_file, $local_file, $mode = FTP_BINARY) {
		$remote_file = jishigou_ftp::clear($remote_file);
		$local_file = jishigou_ftp::clear($local_file);
		$mode = intval($mode);
