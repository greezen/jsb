 ...
 * @version $Id: rewrite.class.php 5543 2014-02-12 08:01:06Z wuliyong $
 */
	...
	function parseRequest($request=null) {
		if($request===null) {
			$request=$this->getRequestUri();
				...
				if(strpos($request,$this->absPath)===0) {
			$request=substr($request,strlen($this->absPath));
		}
		...
		{
			if($this->redirect==false || $_SERVER['REQUEST_METHOD']=='POST') {
				return null;
			}
		...
		if($request==$this->default) {
			return null;
		}
				$request=explode($this->argSeparator,$request);
		if($request[0]==$this->default) {
			array_shift($request);		}
		...
		foreach ($request as $arg)
		{
			...
			if(($_r=$this->valueReplaceList[$var]) && ($_value=array_search($value,$_r))!==false) {
				$value=$_value;
			}
			$value=urldecode($value);
			if($this->varReplaceList && ($_var=array_search($var,$this->varReplaceList))!==false) {
				$var=$_var;
			}
