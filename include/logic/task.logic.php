 ...
 *
 * @Date 2013-11-20 1608250966 6920 $
 */
...
class TaskLogic
{
	...
	function run($id=0)
	{
		...
		{
			$this->task=$task;
			$lock=jconf::get('task','lock');

			$task['filename'] = str_replace(array('..', '/', '\\'), '', $task['filename']);
			$taskfile = './include/ext/task/'.$task['filename'];


			if($lock > ($this->timestamp - 600)) {
				return null;
			} else {
				jconf::set('task','lock',time());
			}
			...
			if(!include_once $taskfile) {
				...
				$log_id=$this->log("执行超时，请检查任务脚本。",E_USER_ERROR,0);
