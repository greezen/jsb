 ...
 *
 * @Date 2013-11-11 730885000 20136 $
 */
...
class TalkLogic
{
	...
	{
		return jtable('topic_mention')->is_at($tid, MEMBER_ID);
	}
	...
	function delete($ids)
	{
		if (!is_array($ids)) {
			$ids = (array)$ids;
				...
				DB::update('topic', array('item'=>'','item_id'=>''), " WHERE `tid` IN (SELECT tid FROM ".DB::table('topic_talk')." WHERE `item`='talk' AND `item_id` IN (".jimplode($ids)."))");
		DB::update('topic', array('type'=>'first'), " WHERE `tid` IN (SELECT tid FROM ".DB::table('topic_talk')." WHERE `item_id` IN (".jimplode($ids).")) AND `type` = 'talk'");
	...
	function update_category_cache()
	{
		...
		while ($value = DB::fetch($query)) {
			...
			jconf::set('talk_category', $cat_ary);
			return $cat_ary;
		}
		return array();
	}
	...
	{
		$cat_ary = array();
		$cat_ary = jconf::get('talk_category');
		if (empty($cat_ary)) {
