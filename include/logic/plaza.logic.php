...
class PlazaLogic {
	const ORDER_TODAY = 0;//今日订单##lbc##
	const ORDER_DIG = 8;//推荐订单##lbc##
	const ORDER_MANUFACTURE = 9;//急需加工的订单##lbc##
	const ORDER_COOPERATION = 10;//长期合作##lbc##
	const ORDER_CHECKED = 11;//已经确认##lbc##
	const ORDER_CHANNEL_ID = 3;
	...
	public function index_topic() {
		...
		$order = array();
		$query = DB::query("SELECT t.tid,t.content,t.imageid,t.digcounts,t.featureid FROM ".DB::table('topic')." t LEFT JOIN ".DB::table('channel')." c ON t.item_id = c.ch_id WHERE c.channel_typeid = 3");
		while ($value = DB::fetch($query))##lbc##
		{
			$temp = explode(',',$value['imageid']);
			$value['imageid'] = $picid[] = $temp[0];			
			$value['content'] = cut_str(strip_tags($value['content']),30,'');
			$featureid = intval($value['featureid']);
			switch ($featureid){
				case self::ORDER_TODAY:
					$order['today_order'][] = $value;
					break;
				case self::ORDER_DIG:
					$order['dig_order'][] = $value;
					break;
				case self::ORDER_MANUFACTURE:
					$order['manfufacture_order'][] = $value;
					break;
				case self::ORDER_COOPERATION:
					$order['cooperation_order'][] = $value;
					break;
				case self::ORDER_CHECKED:
					$order['checked_order'][] = $value;
					break;
			}
		}
		
		$query = DB::query("SELECT t.tid,t.content,t.imageid,t.digcounts,t.featureid,i.* FROM ".DB::table('topic')." t LEFT JOIN ".DB::table('channel')." c ON t.item_id = c.ch_id LEFT JOIN ".DB::table('topic_image')." i ON t.tid=i.tid WHERE c.channel_typeid = 3 ORDER BY i.id DESC LIMIT 1");
		while ($value = DB::fetch($query))
		{
			$temp = explode(',',$value['imageid']);
			$value['imageid'] = $picid[] = $temp[0];			
			$value['content'] = cut_str(strip_tags($value['content']),30,'');
			$order['today_pic'][] = $value;
		}
		$index_topic = array(
	...
	//##lbc##获取当日订单信息
	public function order($type = 'today', $p) {
		$ps = array(
				'cache_time' => max(0, (int) $p['cache_time']),
				'page_url' => $p['page_url'],
		);
		...
		$ch_ids = $this->get_order_channel_ids();
		$ps['cache_key'] = 'topic-new_topic';
		$ps['order'] = ' `dateline` DESC ';
		if ($type == 'today') {
			$ps['where'] = ' `type` IN (' . jimplode(get_topic_type()) . ') AND `featureid`= '.self::ORDER_TODAY.' AND `item_id` IN('.$ch_ids.') ';
		}elseif ($type == 'dig'){
			$ps['where'] = ' `type` IN (' . jimplode(get_topic_type()) . ') AND `featureid`= '.self::ORDER_DIG.' AND `item_id` IN('.$ch_ids.') ';
		}elseif ($type == 'maf'){
			$ps['where'] = ' `type` IN (' . jimplode(get_topic_type()) . ') AND `featureid`= '.self::ORDER_MANUFACTURE.' AND `item_id` IN('.$ch_ids.') ';
		}elseif ($type == 'coop'){
			$ps['where'] = ' `type` IN (' . jimplode(get_topic_type()) . ') AND `featureid`= '.self::ORDER_COOPERATION.' AND `item_id` IN('.$ch_ids.') ';
		}
	...
	//##lbc##获取订单模型的所有频道ID
	private function get_order_channel_ids(){
		$rows = DB::fetch_all('SELECT `ch_id` FROM '.DB::table('channel').' WHERE `channel_typeid` = '.self::ORDER_CHANNEL_ID);
		$ch_ids = array();
		foreach ($rows as $row){
			$ch_ids[] = $row['ch_id'];
		}
		return implode(',', $ch_ids);
	}
