 ...
 * @version $Id: video.logic.php 5099 2013-11-25 09:57:16Z wuliyong $
 */
...
class VideoLogic
{
	...
	function delete($ids) {
		...
		foreach($rets['list'] as $r) {
			$id = (int) $r['id'];
						if($id > 0 && (jallow($r['uid']))) {
								$ret = (jtable($this->table)->delete($id) || $ret);
								jio()->DeleteFile($r['video_img']);
				if($r['tid'] > 0) {
	...
	function parse($url) {
		...
		if(!$ret && $sys_config['video_parse_extract']) {
			$ret = jclass('video_parse_extract')->parse($url);
		}
