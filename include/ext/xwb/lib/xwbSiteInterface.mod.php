 ...
 *
 * @version $Id: xwbSiteInterface.mod.php 3965 2013-07-03 03:07:57Z wuliyong $
 */
...
class xwbSiteInterface {
	...
	function doReg(){
		...
		if ($uid<1){
			...
			if( XWB_plugin::pCfg('is_sync_face') )
			{
				...
				//显示在注册层（方便debug）
				//$msg .= "<br>头像同步结果:  {$faceSyncResultLog}[RETURN CODE: {$faceSyncResult}]";
			}
	...
	function _getBindTip( $code ){
		$tips = array(
			...
			'-201' => '上述帐号昵称已绑定其他新浪微博，更更换其他帐户，或先登录后解除绑定',
		);
