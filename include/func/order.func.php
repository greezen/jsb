 ...
 *
 * @Filename order.func.php $
 *
 * @Author http://www.jishigou.net $
 *
 * @Date 2013-11-11 766731002 3411 $
 */
...
/**
 * 文件名：order.func.php
 * @version $Id: order.func.php 3723 2013-05-28 02:40:03Z wuliyong $
 * 作者：狐狸<foxis@qq.com>
 * 功能描述：排序
 * @version $Id: order.func.php 3723 2013-05-28 02:40:03Z wuliyong $
 */
function __order($order_by_list='',$query_link='',$config=array())
{
	...
	foreach ((array) $order_by_list as $o_by=>$o)
	{
		if((false == isset($o['display']) or $o['display']) and $o_by and isset($o['name']) and ''!=$o['name'])
		{
			...
			if($jishigou_rewrite)
			{
				$href = $jishigou_rewrite->formatURL($href,null);
				if($un_href) $un_href = $jishigou_rewrite->formatURL($un_href,null);
			}
