...
function createtable($sql, $dbcharset, $ex = '') {
	...
	return preg_replace("/^\s*(CREATE TABLE\s+.+\s+\(.+?\)).*$/isU", "\\1", $sql).
		(mysql_get_server_info() > '4.1' ? " ENGINE=$type DEFAULT CHARSET=$dbcharset" : " TYPE=$type").$ex;
}
...
function instfooter() {
	...
	echo "<tr><td><hr noshade align=\"center\" width=\"100%\" size=\"1\"></td></tr>".
        	"<tr><td align=\"center\">".
          	...
          	"</a> &nbsp; Copyright &copy; <a href='http:/'.'/www.cenwor.com' target='_blank'>Cenwor Inc.</a> 2005 - {$SY}</b><br /><br />".
...
function instmsg($message, $url_forward = '') {
	...
	if($url_forward) {
		...
		$message .= "<script type='text/javascript'>setTimeout(\"redirect('$url_forward');\", 1250);</script>";
...
function install_get_system_env( )
{
	...
	if($config['site_name']) $e['sys_name'] = $config['site_name'];
	if($config['site_admin_email']) $e['sys_email'] = $config['site_admin_email'];
	if($config['site_url']) $e['sys_url'] = $config['site_url'];
