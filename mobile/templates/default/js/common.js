...
function makeMBlogList(data)
{
	...
	for (var i=0;i<topic_list.length;++i) {
		...
		//头部
		html += '<article id="weibo_itmes_'+topic['tid']+'" class="weibo" data-tid="'+topic['tid']+'" data-login="1" data-uid="'+topic['uid']+'">';
		
		//用户信息
		html += '<header class="tit-wb clearfix"> <img class="avatar-wb" src="'+topic['face']+'" onclick="goToUserInfo('+ topic['uid'] +');"/> <span class="nikename-wb">'+topic['nickname']+''+topic['validate_string']+'</span><span class="origin-wb"><time class="time-wb" onclick="goToMBlogDetail('+ topic['uid'] +');">'+topic['dateline']+'</time>'+topic['from_string']+'</span>';

		if (!isUndefined(topic['image_list'])) {
			html += '<img src="./images/pic.png"/ style="position: absolute;right: 10px;top: 10px;">';
		}
		html += '</header>';

		//微博正文
		html += '<section class="content-wb"><p>'+topic['content']+'';
		
		//是否含有图片
		if ( !isUndefined(topic['image_list'])) {
			html += '<div class="img-wb">';
			for(var j in topic['image_list']) {
				var image = topic['image_list'][j];
				html += '<img class="author" src="' + image['image_small'] + '" style="width:100px; height:100px;" />';
			}
			html += '</p>';
		}
		...
		//from处理
		html += '<footer class="action-wb">';
		if (topic['forwards'] > 0) {
			html += '<a href="javascript:;" class="rt-action-wb L-item-tab"><p><i>&nbsp;</i><span data-num="0">'+topic['forwards']+'</span></p></a>';
		}
		if (topic['replys'] > 0) {
			html += '<a href="javascript:;" class="cmt-action-wb L-item-tab"><p><i>&nbsp;</i><span data-num="0">'+topic['replys']+'</span></p></a>';
		}
		html += '</footer></article>';
	}
	return html;
...
function makeMemberList(data)
{
	...
	for (i=0;i<member_list.length;i++) {
		...
		if (member['friendship'] == -1) {
			html += '<a href="javascript:;" onclick="delFromBlackLis(this, ' + member['uid'] + ');return true;" class="btn_h"></a>';
		} else if (member['friendship'] == 0) {
			html += '<a href="javascript:;" onclick="addFollow(this, ' + member['uid'] + ');return true;" class="btn_g"></a>';
		} else if (member['friendship'] == 1) {
			html += '自己';
		} else if (member['friendship'] == 2) {
			html += '<a href="javascript:;" onclick="delFollow(this, ' + member['uid'] + ');return true;" class="btn_y"></a>';
...
function goToLookAround()
{
	if (MobileClient) {
		...
		location.href = "index.php?mod=plaza";
	}
}
		...
		function(r) { 
			if (r.code == 200) {
				if (op == "add") {
					...
					$("#btn_fav_mblog").removeClass("wb_fav").addClass("wb_fav_on");
					$("#btn_fav_mblog").html("<p>已收藏</p>");
				} else if (op == "delete") {
					showTipsExp("取消收藏成功", 1);
					$("#btn_fav_mblog").attr("href", "javascript:favMBlog('add', "+tid+");");
					$("#btn_fav_mblog").removeClass("wb_fav_on").addClass("wb_fav");
					$("#btn_fav_mblog").html("<p>已收藏</p");
				}
			} else {
				showTipsExp("收藏失败", 1);
