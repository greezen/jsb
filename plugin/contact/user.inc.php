...
function main(){
	...
	$sql2 = 'SELECT r.*,c.`picture` FROM '.DB::table('channel_buy_record').' r LEFT JOIN '.DB::table('channel').' c ON r.`ch_id`=c.`ch_id` WHERE r.`uid`= '.MEMBER_ID.' AND c.`parent_id`>0 AND r.`state`=1 GROUP BY c.`ch_id` ORDER BY r.`state`,r.`cr_id` DESC ';
